            ...
            <div className="flex items-center justify-between max-w-2xl mx-auto mb-8 relative">
                ...
                {['Upload', 'Map Columns', 'Import'].map((step, idx) => (
                    <div key={step} className="flex flex-col items-center gap-2">
                        <div className={`h-8 w-8 rounded-full flex items-center justify-center font-bold text-sm ${idx === 0 ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'bg-white dark:bg-[#0B1120] border-2 border-slate-200 dark:border-slate-800 text-slate-400'}`}>
                            {idx + 1}
                        </div>
                        <span className={`text-xs font-medium ${idx === 0 ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>{step}</span>
                    </div>
                ))}
            </div>

            {/* Upload Area */}
            <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-12 flex flex-col items-center justify-center text-center bg-slate-50/50 dark:bg-slate-900/20 hover:bg-slate-100/50 dark:hover:bg-slate-900/40 transition-colors cursor-pointer group">
